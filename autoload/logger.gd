...
var default_logfile_path = "res://logs/debug.txt"
...
func get_logfile(logfile_path):
	...
	if not logfiles.has(logfile_path):
		warn("The requested logfile pointing to does not exist." % logfile_path, PLUGIN_NAME)
		return null
	else:
		return logfiles[logfile_path]
...
func set_default_output_strategy(output_strategy_mask, level = -1):
	...
	if level == -1: # Set for all levels
		...
		if not level in range(0, LEVELS.size()):
			error("The level must be comprised between 0 and %d." % str(LEVELS.size() - 1), PLUGIN_NAME)
			return
...
func _init():
	...
	var path_debug = "res://logs/debug.txt"
	var path_warn = "res://logs/warn.txt"
	
	var dir = Directory.new()
	var file = File.new()
	
	if file.file_exists(path_debug):
		dir.remove(path_debug)
	if file.file_exists(path_warn):
		dir.remove(path_warn)
	
	add_logfile(path_debug)
	add_logfile(path_warn)
	# Default modules
	add_module(PLUGIN_NAME) # needs to be instanced first
	add_module("debug", DEBUG, STRATEGY_FILE, get_logfile(path_debug))
	add_module("warn", WARN, STRATEGY_FILE, get_logfile(path_warn))
